        ...
        if prop_name in xaml_property_mapping and prop_value != "none" and prop_name in svg_properties_is_checked:
            if prop_name == "d":
                ...
                for i in range(0, len(svg_properties["d"])):
                    if i == 0:
                        data += f'    <Setter Property="{"Data"}" Value="{svg_properties["d"][i]}'
                    else:
                        data += f' {svg_properties["d"][i]}'
                xaml_code += f'{data}"/>\n'
            elif prop_name == "fill":
...
def main():
    ...
    if choice == "text":
        num_svgs = st.number_input("Enter the number of SVGs", min_value=1, value=1, step=1)
        for i in range(num_svgs):
            input_svg = st.text_area(f"Enter SVG {i+1}", "")
            input_svgs.append(input_svg)
    elif choice == "svg file":
        num_svgs = st.number_input("Enter the number of SVG files", min_value=1, value=1, step=1)
        for i in range(num_svgs):
            uploaded_file = st.file_uploader(f"Upload SVG file {i+1}", type=['svg'])
    ...
    if input_svgs:
        for i, input_svg in enumerate(input_svgs):
            # SVGの解析
            ...
            show_xaml = st.checkbox(f"Show XAML for SVG {i+1}", False)
            if show_xaml:
                st.write(svg_properties)

            for j in ignore_properties:
                if j in svg_properties:
                    del svg_properties[j]
            svg_properties_is_checked = st.multiselect(f"Select XAML properties to convert for SVG {i+1}", list(svg_properties.keys()), list(svg_properties.keys()))
            change = False
            if change != st.button(f"Convert SVG {i+1}"):
                # XAMLコードの生成
                change = not change
                result_xaml += generate_xaml(file_names[i], svg_properties, xaml_propaty, svg_properties_is_checked)
